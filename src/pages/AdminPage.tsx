...
const [biryaniClosed, setBiryaniClosed] = useState(false); // ✅ NEW
  ...
  useEffect(() => {
    ...
    const fetchAdminState = async () => {
      ...
      if (data) {
        ...
        setBiryaniClosed(data.biryani_active ? !!data.biryani_closed : false); // ✅ NEW
      } else {
        // ✅ NEW: Check yesterday's record to carry over biryani_active state
        ...
        const { data: prevData } = await supabase
          .from("admin_state")
          .select("biryani_active")
          .eq("date", yesterdayStr)
          .maybeSingle();

        const carryBiryani = prevData?.biryani_active ?? false;
        ...
        await supabase
          ...
          .upsert(
            [{
              ...
              canteen_open: false,
              menu_items: [],
              biryani_active: carryBiryani, // ✅ carry over from yesterday
              biryani_end: false,
              biryani_closed: false,         // ✅ NEW: reset closed for new day
            }],
            { onConflict: "date" }
          );
  ...
  const handleToggleCanteen = () => {
    ...
    if (canteenOpen) {
      setSelectedMenu([]);
      // ❗ DO NOT touch biryani here — biryani persists across open/close
    }
  };

  const handleToggleBiryani = async () => {
    if (biryaniActive) {
      // 🔥 clear biryani orders ONLY when admin turns OFF
      await supabase
        .from("orders")
        .delete()
        .eq("menu", "Biryani")
      ...
      setBiryaniEnd(false);
      setBiryaniClosed(false); // ✅ NEW: also reset closed
    }

    setBiryaniActive(prev => !prev);
  };

  const handleToggleBiryaniEnd = () => {
    if (!biryaniActive) return;
    setBiryaniEnd(prev => !prev);
  };

  // ✅ NEW: Toggle biryani closed (ordering blocked)
  const handleToggleBiryaniClosed = () => {
    if (!biryaniActive) return;
    setBiryaniClosed(prev => !prev);
  };
      ...
      .upsert([
        {
          ...
          biryani_closed: biryaniClosed, // ✅ NEW
        },
      ], { onConflict: "date" });
    setShowSaved(true);
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", padding: "16px 16px 90px" }}>
          ...
          {canteenOpen && (
            <>
              <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                ...
                {/* Biryani buttons column */}
                <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 45 }}>
                  {/* Biryani ON/OFF */}
                  <button
                    style={{
                  ...
                  {/* ✅ NEW: Biryani Closed button */}
                  <button
                    disabled={!biryaniActive}
                    style={{
                      background: biryaniClosed ? "#6f42c1" : "#eee",
                      padding: 10,
                      borderRadius: 8,
                      border: "none",
                      fontWeight: 700,
                      width: 120,
                      color: biryaniClosed ? "#fff" : "#333",
                      opacity: !biryaniActive ? 0.5 : 1,
                      cursor: !biryaniActive ? "not-allowed" : "pointer",
                    }}
                    onClick={handleToggleBiryaniClosed}
                  >
                    {biryaniClosed ? "Orders Closed" : "Close Orders"}
                  </button>
                </div>
              </div>
            </>
              ...
              {biryaniOrders.map((order, idx) => (
                ...
                >
                  ...
                  <td className="col-action">
                    <button
                      className={`order-btn ${order.completed ? "unselect" : "complete"}`}
                      onClick={() =>
                        order.completed
      ...
      <footer className="admin-footer-mobile">
        ...
        <button className="footer-btn refresh-btn" onClick={refreshOrders}>Refresh</button>
